...
public:
    enum JoystickErrorCodes
    {
        ...
        jerr_controller_create_error = 0x04,
        jerr_unknown_controller_type = 0x10
    };

public:
    Joystick(unsigned int device_number = 0) : device_number_(device_number) {}
    ~Joystick();
...
public:
    typedef nonstd::function<void(JoystickController *joystick_controller)> CreateHandler;
...
private:
    PS2X control_;
    JoystickController *p_jimpl_ = nullptr;
